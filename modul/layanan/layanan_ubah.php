<?php
	if(isset($_POST['btnSave'])){
		...
		if (trim($_POST['txtNama'])=="") {
			$message[] = "<b>Nama Layanan</b> tidak boleh kosong!";		
		}
		...
		if(count($message)==0){	
			$qryUpdate=mysqli_query($koneksidb, "UPDATE ms_layanan SET nama_layanan='$txtNama',
							  							keterangan_layanan='$txtKeterangan',
							  							status_layanan='$cmbStatus',
														updatedBy='$updatedBy',
														updatedTime='".date('Y-m-d')."'
												WHERE id_layanan ='".$_POST['txtKode']."'") 
					   or die ("Gagal query update".mysqli_error());
			if($qryUpdate){
				$_SESSION['pesan'] = 'Data Layanan berhasil diperbaharui';
	...
	$sqlShow 		= "SELECT * FROM ms_layanan WHERE id_layanan='$KodeEdit'";
	$qryShow 		= mysqli_query($koneksidb, $sqlShow)  or die ("Query ambil data Layanan salah : ".mysqli_error());
	$dataShow 		= mysqli_fetch_array($qryShow);
	
	$dataKode		= $dataShow['id_layanan'];
...
<div class="portlet box grey-cascade">
	<div class="portlet-title">
		<div class="caption"><span class="caption-subject uppercase bold">Form Ubah Kegiatan</span></div>
		<div class="tools">
